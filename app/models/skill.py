...
relevant executor (app/knowledge/models.py for KNOWLEDGE, app/uiaction
for UI), never here.

AgentX v2 (see alembic/versions/0010_ui_skill_type.py): adds skill_type
UI, for Skills that propose structured UI Actions (app.uiaction) rather
than calling MCP or retrieving Knowledge - same additive-enum pattern
KNOWLEDGE used, gated end-to-end by FEATURE_UI_SKILLS.
"""
...
class SkillType(str, Enum):
    ...
    KNOWLEDGE  - executed by app.knowledge.executor.KnowledgeSkillExecutor
                 ...
                 entirely configured via `config`.
    UI         - executed by app.uiaction.service.UIActionSkillService;
                 lets an Agent propose a structured UIActionPlan
                 (app.uiaction.models) instead of / alongside text.
                 Only meaningful when FEATURE_UI_SKILLS is enabled.
    """
